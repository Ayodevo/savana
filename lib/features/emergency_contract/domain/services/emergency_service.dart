...
class EmergencyService implements EmergencyServiceInterface{
  ...
  Future addNewEmergencyContact(String name, String phone, int? id, {bool isUpdate = false}) async {
    ApiResponse response = await emergencyContractRepoInterface.addNewEmergencyContact(name, phone, id);
    if(response.response!.statusCode == 200) {
      isUpdate ? showCustomSnackBarWidget(getTranslated("contact_updated_successfully", Get.context!), Get.context!, isError: false):
