           ...
           builder: (context,resProvider, _) {
             ...
             return!resProvider.isLoading? SizedBox(height: 50,
               ...
               onTap: (){
                 ...
                 if(haveBlankTitle){
                   showCustomSnackBarWidget(getTranslated('please_input_all_title',context),context);
                 }else if(haveBlankDes){
                   showCustomSnackBarWidget(getTranslated('please_input_all_des',context),context);
                 }
                   ...
                   showCustomSnackBarWidget(getTranslated('please_choose_digital_product',context),context);
                 }
                 else if (resProvider.categoryIndex == 0) {
                   showCustomSnackBarWidget(getTranslated('select_a_category',context),context);
                 }
                 else if (resProvider.brandIndex == 0 && Provider.of<SplashController>(context, listen: false).configModel!.brandSetting == "1") {
                   showCustomSnackBarWidget(getTranslated('select_a_brand',context),context);
                 }
                 else if (resProvider.unitValue == '' || resProvider.unitValue == null &&  resProvider.productTypeIndex == 0) {
                   showCustomSnackBarWidget(getTranslated('select_a_unit',context),context);
                 }
                 else if (resProvider.productCode.text == '' || resProvider.productCode.text.isEmpty) {
                   showCustomSnackBarWidget(getTranslated('product_code_is_required',context),context);
                 }
                 else if (resProvider.productCode.text.length < 6 || resProvider.productCode.text == '000000') {
                   showCustomSnackBarWidget(getTranslated('product_code_minimum_6_digit',context),context);
                 }
                 else{
